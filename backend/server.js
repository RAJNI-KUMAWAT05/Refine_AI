...
app.get("/auth/google", async (req, res, next) => {
    console.log("➡️ /auth/google called. process.env.GOOGLE_CLIENT_ID:", process.env.GOOGLE_CLIENT_ID);
    if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_ID.startsWith("dummy")) {
        try {
            console.log("⚡ Dummy auth mode active");
            let user = await User.findOne({ googleId: "dummy-google-id" });
            if (!user) {
                console.log("📝 Creating dummy developer guest user in MongoDB...");
                user = await User.create({
            ...
            console.log("👤 Dummy user details:", user);
            req.login(user, (err) => {
                if (err) {
...
console.log('🚀 Groq integration initializing');
console.log('   Model    :', GROQ_MODEL);
console.log('   API key  :', process.env.GROQ_API_KEY ? '✅ present' : '❌ missing');

// In‑memory cache for refinement results (key: requirement string)
...
async function callGroq(groq, systemInstruction, prompt) {
...
async function runRound({ groq, round, initialRequirement, priorSummary }) {
    ...
    const agentResults = await Promise.all(
        AGENTS.map(async (agent) => {
            ...
            let raw;
            try {
                raw = await callWithRetry(() => callGroq(groq, agent.system, agentPrompt));
            } catch (err) {
    ...
    let judgeRaw;
    try {
        judgeRaw = await callWithRetry(() => callGroq(groq, JUDGE.system, judgePrompt));
    } catch (err) {
...
app.post("/feedback", async (req, res) => {
    const { feedback, priorContext, projectId, requirement } = req.body;
    ...
    const updatedProject = await Project.findByIdAndUpdate(
        projectId,
        {
            ...
            requirement: requirement || feedback || "",
...
app.post("/refine", async (req, res) => {
  console.log('🔍 /refine payload:', req.body);
  ...
  // Check cache first
  if (refineCache.has(requirement)) {
    console.log('🔁 Cache hit for requirement');
    const cached = refineCache.get(requirement);
  ...
  // Store in cache (simple TTL of 5 minutes)
  refineCache.set(requirement, responsePayload);
  setTimeout(() => refineCache.delete(requirement), 5 * 60 * 1000);
